...
def docking(data):
    ...
    if data.pose.position.z > 20.00 :
        if data.pose.position.x > -6: 
            cmd.angular.z = -0.08
            # print('go right')
        elif data.pose.position.x < -8: 
            cmd.angular.z = 0.08
            # print('go left')
...
def go_back_charging_station():
    ...
    #                             Quaternion(0, 0, 0, 1))
    goal.target_pose.pose.position.x=1.489
    goal.target_pose.pose.position.y=3.819
